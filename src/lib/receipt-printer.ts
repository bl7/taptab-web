...
export class ReceiptPrinter {
  ...
  connect(jwtToken: string) {
    ...
    if (this.connectionAttempts >= this.maxConnectionAttempts) {
      console.warn(
        "⚠️ Maximum connection attempts reached, stopping connection attempts"
      );
      return;
    }
    ...
    const wsUrl =
      process.env.NEXT_PUBLIC_WEBSOCKET_URL || "http://localhost:5050";

    console.log(
      `🔌 Creating new WebSocket connection to ${wsUrl}... (attempt ${this.connectionAttempts}/${this.maxConnectionAttempts})`
    );
    // Connect to WebSocket server
  ...
  private setupEventListeners() {
    ...
    this.socket.on("newOrder", (data: WebSocketOrderEvent) => {
      ...
      console.log("📋 Event details:", {
        ...
        total:
          (data.order as BackendOrderData)?.total || data.order?.finalAmount,
      });
      ...
      if (data.type === "PRINT_RECEIPT") {
        ...
        console.log(
          "⚠️ Ignoring newOrder event with unexpected type:",
          data.type
        );
      }
    });
    ...
    this.socket.on("orderModified", (data: WebSocketOrderEvent) => {
      ...
      if (data.type === "PRINT_MODIFIED_RECEIPT") {
        ...
        this.processModifiedOrderNotification(
          data.order,
          data.changes,
          data.notificationId
        );
        ...
        console.log(
          "⚠️ Ignoring orderModified event with unexpected type:",
          data.type
        );
      }
    });
  ...
  private async processNewOrderNotification(
    orderData: OrderData,
  ...
  private async processModifiedOrderNotification(
    orderData: OrderData,
    changes?: OrderChanges,
    notificationId?: string
  ) {
  ...
  private isNotificationProcessed(notificationId: string): boolean {
    ...
    if (isRecentlyProcessed) {
      console.log(
        "⚠️ Notification processed recently:",
        notificationId,
        `(${Math.round(timeSinceProcessed / 1000)}s ago)`
      );
      return true;
    }
  ...
  private mapBackendOrderData(orderData: OrderData): OrderData {
    ...
    console.log("LOOKFORTHIS 🔍 Waiter Data Debug:", {
      ...
      fallbackName:
        orderData.waiterName || orderData.sourceDetails || "Unknown Waiter",
    ...
    return {
      ...
      tableNumber: orderData.tableNumber,
      totalAmount:
        orderData.totalAmount || (orderData as BackendOrderData).total || 0,
      finalAmount:
        orderData.finalAmount || (orderData as BackendOrderData).total || 0,
  ...
  private showBrowserNotification(orderData: OrderData) {
    if ("Notification" in window && Notification.permission === "granted") {
      ...
      new Notification("New Order Received!", {
        body: `Order #${orderData.orderNumber} - Table ${
  ...
  private showModifiedBrowserNotification(
    orderData: OrderData,
    changes?: OrderChanges
  ) {
    if ("Notification" in window && Notification.permission === "granted") {
      ...
      let notificationBody = `Order #${orderData.orderNumber} - Table ${
        orderData.tableNumber
      }\n\nComplete Order:\n${itemsList}\n\nTotal: $${orderData.finalAmount.toFixed(
        2
      )}`;
      ...
      if (changes) {
        ...
        if (changes.addedItems && changes.addedItems.length > 0) {
          notificationBody += `\n➕ Added: ${changes.addedItems
            .map((item) => `${item.name} x${item.quantity}`)
            .join(", ")}`;
        }

        if (changes.removedItems && changes.removedItems.length > 0) {
          notificationBody += `\n➖ Removed: ${changes.removedItems
            .map((item) => `${item.name} x${item.quantity}`)
            .join(", ")}`;
        }

        if (changes.modifiedItems && changes.modifiedItems.length > 0) {
          notificationBody += `\n✏️ Modified: ${changes.modifiedItems
            .map(
              (item) => `${item.name} ${item.oldQuantity}→${item.newQuantity}`
            )
            .join(", ")}`;
        }
      }
  ...
  markAsRead(notificationId: string) {
    ...
    if (notificationIndex !== -1) {
      ...
      console.log(
        "⚠️ Notification not found for marking as read:",
        notificationId
      );
    }
  }
  ...
  getPrintBridgeStatus(): { connected: boolean; readyState?: number } {
    return {
      connected: !!(
        this.printBridgeWebSocket &&
        this.printBridgeWebSocket.readyState === WebSocket.OPEN
      ),
  ...
  private async printReceipt(orderData: OrderData, changes?: OrderChanges) {
    try {
      console.log(
        "🖨️ Starting receipt printing process for order:",
      ...
      const receiptDataURL = await this.receiptGenerator.generateReceiptPNG(
        orderData,
        changes
      );
      ...
      const actualHeightPixels = this.receiptGenerator.getHeight(
        orderData,
        changes
      );
      ...
      if (
        ...
        console.log(
          "💡 To enable receipt printing, ensure PrintBridge server is running on localhost:8080"
        );
      }
    } catch (error) {
  ...
  testNotification() {
    ...
    this.processNewOrderNotification(
      testOrder,
      `test_notification_${Date.now()}_${Math.random()
        .toString(36)
        .substr(2, 9)}`
    );
  }
