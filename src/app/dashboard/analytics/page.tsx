...
export default function AnalyticsPage() {
  ...
  const [timeRange, setTimeRange] = useState<'yesterday' | 'week' | 'month' | 'year'>('week');
  ...
  useEffect(() => {
    const loadAnalytics = async () => {
      try {
        ...
        const [comprehensive, peakHours, topItems, staffPerformance] = await Promise.all([
          ...
          api.getStaffPerformance(timeRange === 'yesterday' ? 'week' : timeRange)
        ]);

        console.log('thisbitch', 'Analytics data loaded:', { comprehensive, peakHours, topItems, staffPerformance });
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-green-600">+{analytics?.comprehensive.growth.revenue.toFixed(1)}%</span>
        </div>
      </div>
      
      <div className="h-48 flex items-end justify-between space-x-1">
        {analytics?.comprehensive.dailyData.map((day, index) => {
          const maxRevenue = Math.max(...analytics.comprehensive.dailyData.map(d => d.revenue));
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm">
      ...
      <div className="grid grid-cols-12 gap-1">
        {analytics?.peakHours.peakHours.map((hour, index) => {
          const maxOrders = Math.max(...analytics.peakHours.peakHours.map(h => h.orderCount));
          ...
          <div key={index} className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center">
                <span className="text-sm font-bold text-green-600">{item.rank}</span>
              </div>
              <div>
      ...
      <div className="space-y-3">
        {analytics?.staffPerformance.map((staff, index) => (
          <div key={index} className="flex items-center justify-between p-3 border border-gray-100 rounded-lg">
            ...
            <div className="text-right">
              <p className="text-sm font-semibold text-gray-900">{staff.orderCount} orders</p>
              <p className="text-xs text-gray-500">${staff.totalRevenue.toFixed(2)}</p>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-black">View your restaurant&apos;s performance metrics</p>
        </div>
        <div className="flex items-center space-x-4">
          ...
          <select 
            value={timeRange} 
            onChange={(e) => {
              setTimeRange(e.target.value as 'yesterday' | 'week' | 'month' | 'year');
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">${analytics.comprehensive.summary.totalRevenue.toFixed(2)}</p>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{analytics.comprehensive.summary.totalOrders}</p>
                <div className="flex items-center mt-1">
                  <TrendingUp className="w-4 h-4 text-green-600 mr-1" />
                  <span className="text-sm text-green-600">+{analytics.comprehensive.growth.orders.toFixed(1)}%</span>
                </div>
              </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">${analytics.comprehensive.summary.avgOrderValue.toFixed(2)}</p>
                <div className="flex items-center mt-1">
                  <TrendingUp className="w-4 h-4 text-green-600 mr-1" />
                  <span className="text-sm text-green-600">+{analytics.comprehensive.growth.avgOrderValue.toFixed(1)}%</span>
                </div>
              </div>
