...
export const TestPrintButton: React.FC = () => {
  ...
  const testPrint = async () => {
    if (!isConnected) {
      showToast.error(
        "PrintBridge not connected. Make sure the server is running on localhost:8080"
      );
      return;
    }
    ...
    try {
      ...
      if (success) {
        ...
        showToast.success("Test print job sent! Check your printer.");
      } else {
        console.error("❌ Failed to send test print job");
        showToast.error("Failed to send print job. Check console for details.");
      }
    } catch (error) {
      console.error("❌ Error testing print:", error);
      showToast.error("Error testing print. Check console for details.");
    }
  };
