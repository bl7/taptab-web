...
export default function OrderSplitModal({
  ...
  // Simple equal split by amount (for manual 2-way split)
  const splitEvenly = (ways: number) => {
    if (!order) return;

    const orderTotal = order.totalAmount || order.total || 0;
    const targetAmount = orderTotal / ways;
    console.log(
      `🎯 Split Evenly (${ways} ways): Target $${targetAmount.toFixed(2)} each`
    );

    const splitItems = calculateOptimalSplit(targetAmount);
    setSelectedItems(splitItems);
    setReason(`Split evenly (${ways} ways)`);
  };

  // Multi-split functions that create multiple new orders
  const splitIntoThreeOrders = async () => {
    if (!order) return;

    const orderTotal = order.totalAmount || order.total || 0;
    const targetPerOrder = orderTotal / 3;

    console.log(
      `🎯 Multi-Split: Creating 3 orders of $${targetPerOrder.toFixed(2)} each`
    );

    try {
      setLoading(true);

      // First split - create order with first ⅓
      const firstSplit = calculateOptimalSplit(targetPerOrder);
      const firstResult = await api.splitOrder(order.id, {
        itemsToSplit: firstSplit.map((si) => ({
          itemId: si.itemId,
          quantity: si.quantity,
        })),
        customerName: `${order.customerName || "Customer"} - Bill 1`,
        reason: "Split into 3 equal orders (1/3)",
      });

      // Second split - from the updated source order
      const updatedOrder = firstResult.data.updatedSourceOrder;
      const secondSplit = calculateOptimalSplit(
        targetPerOrder,
        updatedOrder.items
      );
      const secondResult = await api.splitOrder(updatedOrder.id, {
        itemsToSplit: secondSplit.map((si) => ({
          itemId: si.itemId,
          quantity: si.quantity,
        })),
        customerName: `${order.customerName || "Customer"} - Bill 2`,
        reason: "Split into 3 equal orders (2/3)",
      });

      console.log(`✅ Created 3 orders:`, {
        order1: firstResult.data.newOrder.orderNumber,
        order2: secondResult.data.newOrder.orderNumber,
        original: secondResult.data.updatedSourceOrder.orderNumber,
      });

      alert(
        `Successfully created 3 equal orders!\n- ${firstResult.data.newOrder.orderNumber}: $${firstResult.data.splitDetails.totalSplitAmount}\n- ${secondResult.data.newOrder.orderNumber}: $${secondResult.data.splitDetails.totalSplitAmount}\n- Original: $${secondResult.data.splitDetails.sourceOrderRemainingTotal}`
      );
      ...
      console.error("❌ Multi-split failed:", error);
      setError(
        `Failed to create multiple orders: ${
  ...
  const splitIntoFourOrders = async () => {
    if (!order) return;

    const orderTotal = order.totalAmount || order.total || 0;
    const targetPerOrder = orderTotal / 4;

    console.log(
      `🎯 Multi-Split: Creating 4 orders of $${targetPerOrder.toFixed(2)} each`
    );
    ...
    try {
      ...
      // Create 3 new orders, leaving ¼ in original
      for (let i = 1; i <= 3; i++) {
        const split = calculateOptimalSplit(targetPerOrder, currentOrder.items);
        const result = await api.splitOrder(currentOrder.id, {
          itemsToSplit: split.map((si) => ({
            itemId: si.itemId,
            quantity: si.quantity,
          })),
          customerName: `${order.customerName || "Customer"} - Bill ${i}`,
          reason: `Split into 4 equal orders (${i}/4)`,
        });

        newOrders.push(result.data.newOrder);
        currentOrder = result.data.updatedSourceOrder;
      }

      console.log(`✅ Created 4 orders:`, {
        ...newOrders.reduce(
          (acc, order, i) => ({ ...acc, [`order${i + 1}`]: order.orderNumber }),
          {}
        ),
        original: currentOrder.orderNumber,
      });

      alert(
        `Successfully created 4 equal orders!\n${newOrders
          .map((o) => `- ${o.orderNumber}: $${o.total || o.totalAmount}`)
          .join("\n")}\n- Original: $${
